...
Write-Log "   BLOCK ALL .EXE IN FOLDER V 0.3   " -Color Cyan
...
if (-not $SkipCheck) {
    ...
    try {
        ...
        try {
            $allRules = Get-NetFirewallRule -Action Block -Enabled True -ErrorAction Stop
            
            if ($allRules -and $allRules.Count -gt 0) {
                $allAppFilters = $allRules | Get-NetFirewallApplicationFilter -ErrorAction Stop
                
                for ($i = 0; $i -lt $allRules.Count; $i++) {
                    $rule = $allRules[$i]
                    $appFilter = $allAppFilters[$i]
                    ...
                    if ($appFilter -and $appFilter.Program -and $appFilter.Program -ne 'Any') {
                        ...
                        if ($rule.Direction -eq 'Inbound') {
                            $blockRulesLookup[$programPath]['Inbound'] = $true
                        }
                        elseif ($rule.Direction -eq 'Outbound') {
                            $blockRulesLookup[$programPath]['Outbound'] = $true
                        }
                    }
                }
            }
